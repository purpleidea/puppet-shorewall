...
RELEASE = 1
SPEC = rpmbuild/SPECS/puppet-shorewall.spec
...
docs: puppet-shorewall-documentation.pdf

puppet-shorewall-documentation.pdf: DOCUMENTATION.md
	pandoc DOCUMENTATION.md -o 'puppet-shorewall-documentation.pdf'
...
$(RPM): $(SPEC) $(SOURCE)
	...
	mv rpmbuild/RPMS/noarch/puppet-shorewall-$(VERSION)-$(RELEASE).*.rpm $(RPM)
...
$(SRPM): $(SPEC) $(SOURCE)
	...
	#mv rpmbuild/SRPMS/puppet-shorewall-$(VERSION)-$(RELEASE).*.src.rpm $(SRPM)

#
#	spec
#
$(SPEC): rpmbuild/ puppet-shorewall.spec.in
	@echo Running templater...
	#cat puppet-shorewall.spec.in > $(SPEC)
	sed -e s/__VERSION__/$(VERSION)/ -e s/__RELEASE__/$(RELEASE)/ < puppet-shorewall.spec.in > $(SPEC)
...
$(SOURCE): rpmbuild/
	...
	git archive --prefix=puppet-shorewall-$(VERSION)/ -o $(SOURCE) $(VERSION) 2> /dev/null || (echo 'Warning: $(VERSION) does not exist.' && git archive --prefix=puppet-shorewall-$(VERSION)/ -o $(SOURCE) HEAD)
